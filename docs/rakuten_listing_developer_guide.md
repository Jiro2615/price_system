...
- `効果 / 効能 / 防ぐ / 改善 / 予防` など単語だけの強制NGを廃止し、治療・消失・効果保証などの表現パターンに分ける。`保湿効果`、`肌の乾燥を防ぐ`、`メイクアップ効果で小ジワを目立たなく見せる`、`乾燥による小ジワを目立たなくする`、`ニキビを防ぐ`、`潤滑効果` はこの表現フィルターでは通過する。
- `ニキビを治す / シミを消す / 病気を治す / 糖尿病を改善 / 絶対に効果がある` などはNG。直後の否定や、限定された見かけ上のメイク効果はその一致箇所に限って扱い、別文のNG表現まで免除しない。その他、既存の生理作用表現（疲労回復、脂肪燃焼等）は引き続き検出する。
- 新しいブランド一致・NG表現は「同一JANが多数出品されている」という理由で免除しない。医薬部外品という区分だけでもNG表現を免除しない。明示的な条件無視出品の選択仕様は変更しない。
- `公式` やその他の禁止語、ブラックリスト、過去NG、販売可能な商品区分・証跡・配送条件等は変更しない。DBの `match_mode` 値自体は変更しない（旧 `contains` の6語を出品評価層で限定解釈）。このポリシーは削除候補の検索には適用しない。

これは運用上の表現フィルターであり、広告の適法性や商品ごとの効能・承認を証明する機能ではない。根拠が必要な表現について根拠を取得したものと扱わない。化粧品の効能範囲や限定条件は[厚生労働省の通知](https://www.mhlw.go.jp/web/t_doc?dataId=00tb7518&dataType=1)および[医薬品等適正広告基準の解説](https://www.mhlw.go.jp/file/06-Seisakujouhou-11120000-Iyakushokuhinkyoku/0000179263.pdf)を参照する。パターン追加時には非医療用途や否定文の誤検知もテストする。

**過去NGは自動解除しない。** 既存のNG記録は初期除外されるため、今回の変更だけでは過去のASINが自動で再出品されることはない。再確認候補の抽出は次の読み取り専用コマンドを使う。

```powershell
py -3.12 C:\rakuten\price_system_listing\scripts\preview_listing_text_policy_past_ng.py --store rakuten_2 --summary-only
py -3.12 C:\rakuten\price_system_listing\scripts\preview_listing_text_policy_past_ng.py --store rakuten_2 --output C:\rakuten\price_system_listing\output\text_policy_past_ng_review.json
```
