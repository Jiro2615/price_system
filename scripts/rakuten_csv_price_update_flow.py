...
# WindowsのPowerShell/cmdで子プロセス出力をPIPE取得すると、cp932系になることが多い。
# UTF-8固定で読むと「現状維持」などが文字化けするため、OS既定エンコーディングで読む。
...
def py_cmd(script_name: str) -> list[str]:
    # -u で子プロセスの標準出力を非バッファ化し、親フロー上でも進捗をすぐ表示する。
...
def build_export_args(args, output_csv: Path, check_csv: Path) -> list[str]:
    cmd = [
        *py_cmd("export_rakuten_normal_item_price_csv.py"),
        "--store",
        args.store,
        "--limit",
        str(args.limit),
...
def apply_success(csv_path: Path, args) -> None:
    cmd = [
        *py_cmd("apply_rakuten_csv_success_to_db.py"),
        "--csv",
        str(csv_path),
        "--execute",
    ]
...
def apply_result(csv_path: Path, log_path: Path, args) -> None:
    cmd = [
        *py_cmd("apply_rakuten_csv_result_to_db.py"),
        "--csv",
        str(csv_path),
        "--log",
        str(log_path),
