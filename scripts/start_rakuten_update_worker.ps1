...
function Write-IniFile {
    ...
    $lines = @(
        "# Rakuten update worker launcher settings",
        "# This file is auto-updated by start_rakuten_update_worker.ps1",
        "Store=$($Data.Store)",
...
function Prompt-Int {
    ...
    while ($true) {
        ...
        Write-Host "Invalid value. Minimum is $Minimum."
    }
}
...
$iniPath = Join-Path $ProjectDir "config\rakuten_worker_launcher.ini"
$lockPath = Join-Path $ProjectDir "config\rakuten_worker_launcher.lock.json"
...
$workerId = "$hostName-$Store"

$scriptsDir = Join-Path $ProjectDir "scripts"
$workerScript = Join-Path $scriptsDir "rakuten_update_worker_loop.py"
$logsDir = Join-Path $ProjectDir "logs\rakuten_update_worker"
...
if ($lockInfo -and $lockInfo.pid) {
    if (Test-ProcessId -Id ([int]$lockInfo.pid)) {
        if (-not $Force) {
            throw "Rakuten update worker appears to be running already. Use -Force only if the lock is stale."
        }
    } else {
...
$logPath = Join-Path $logsDir ("rakuten_update_worker_{0}_{1}.log" -f $Store, $timestamp)
...
Write-Host '===== Rakuten update worker ====='
...
Write-Host '===== Rakuten update worker stopped. Press Enter to close. ====='
Read-Host
exit `$exitCode
"@
...
Write-Host ""
Write-Host "===== Rakuten update worker launcher ====="
Write-Host "mode         : $(if ($Once) { 'EXECUTE ONCE' } else { 'EXECUTE LOOP' })"
