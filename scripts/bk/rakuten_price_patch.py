...
def call_item_patch(manage_number: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not (200 <= res.status_code < 300):
        ...
        raise RuntimeError(f"楽天価格更新APIエラー status={res.status_code}: {data}")
...
def call_item_get(manage_number: str) -> dict[str, Any]:
    url = item_url(manage_number)
    headers = load_auth_header()
    ...
    if not (200 <= res.status_code < 300):
        print(json.dumps(data, ensure_ascii=False, indent=2))
        raise RuntimeError(f"楽天items.getエラー status={res.status_code}: {data}")
    ...
    try:
        for index, row in enumerate(rows, start=1):
            ...
            try:
                ...
                patch_response = call_item_patch(manage, request_payload)

                verified_price: int | None = None
                verify_response: dict[str, Any] | None = None

                if not args.no_verify:
                    verify_response = call_item_get(manage)
