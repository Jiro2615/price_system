...
def make_normal_item_rows(rows: list[dict[str, Any]], include_stock: bool, include_product_rows: bool) -> list[list[Any]]:
    """
    normal-item.csv用の行を生成する。

    楽天の行判定:
    - 商品レベル行: 商品管理番号あり、選択肢タイプなし、SKU管理番号なし
    - SKUレベル行: 商品管理番号あり、SKU管理番号あり、選択肢タイプなし

    価格更新はSKUレベル行に通常購入販売価格を入れる。
    include_product_rows=True の場合、商品ごとに空の商品レベル行も先に出す。
    """
...
def write_normal_item_csv(path: Path, rows: list[dict[str, Any]], include_stock: bool, include_product_rows: bool) -> None:
    ...
    # 楽天 normal-item.csv はRMS側でShift-JIS/CP932前提のためCP932で出力する。
...
def main() -> int:
    parser = argparse.ArgumentParser(description="楽天 normal-item.csv 形式の価格更新CSVをDBから出力します。")
    ...
    if args.output:
        ...
        # 楽天側で商品更新CSVと認識されるよう、ファイル名の先頭は normal-item にする。
    ...
    print("")
    print("まずは1件〜5件で手動アップロードして、RMS側の取込結果を確認してください。")
    print("通れば、次にSFTPアップロード自動化へ進めます。")
