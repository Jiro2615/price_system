...
URL: https://webservice.rms.rakuten.co.jp/merchant-portal/view/ja/common/1-1_service_index/rakutenpayorderapi/updateorderdelivery
サービス: 楽天ペイ受注API（RakutenPayOrderAPI）

サービス一覧へ戻る / RakutenPayOrderAPI

RMS WEB SERVICE : updateOrderDelivery
Overview

この機能を利用すると、楽天ペイ注文の「配送方法の更新」を行うことができます。こちらは同期処理となります。
...
1	注文番号	orderNumber	yes	String	382	-		290333-20171006-10640141
2	配送方法	deliveryName	yes	String	192	-	店舗設定で設定した配送方法を指定して下さい。	宅配便
Response
HTTP Status
...
Response in JSON format (Status: 200 OK)
{
    "MessageModelList": [
        {
            "messageType": "INFO",
            "messageCode": "ORDER_EXT_API_UPDATE_ORDERDELIVERY_INFO_101",
            "message": "配送方法の更新設定が完了しました。",
            "orderNumber": "123456-20180101-1241583267"
        }
    ]
}
...
Response in JSON format (Status: 400 Bad Request)
{
    "MessageModelList": [
        {
            "messageType": "ERROR",
            "messageCode": "ORDER_EXT_API_UPDATE_ORDERDELIVERY_ERROR_009",
            "message": "deliveryNameの項目を指定して下さい。",
            "orderNumber": "123456-20180101-1241583267"
        }
    ]
}
