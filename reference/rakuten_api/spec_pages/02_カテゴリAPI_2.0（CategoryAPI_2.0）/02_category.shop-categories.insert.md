...
URL: https://webservice.rms.rakuten.co.jp/merchant-portal/view/ja/common/1-1_service_index/categoryapi2/insertcategory/
サービス: カテゴリAPI 2.0（CategoryAPI 2.0）

サービス一覧へ戻る / CategoryAPI 2.0

RMS WEB SERVICE : category.shop-categories.insert
Overview
この機能を利用すると、カテゴリ情報を登録することができます。
ここで登録したカテゴリは category.category-trees.upsert でカテゴリツリーに登録してください。カテゴリツリーに登録されるまではAPIやRMSでそのカテゴリを取得できません。
カテゴリツリーに登録されていないカテゴリは、最終更新日から24時間以降に削除します。

Endpoint / HTTP Method
Endpoint	HTTP Method
https://api.rms.rakuten.co.jp/es/2.0/categories/shop-categories	POST
Request
HTTP Header
No	Key	Value
...
0-9、a-z、A-Z
- (U+002D)、. (U+002E)、/ (U+002F)、: (U+003A)、_ (U+005F)
...
1	categoryId	カテゴリID	yes	string	40	1	作成されたカテゴリID。
...
        {
            "code": "IE0107",
            "message": "CategorySet data does not exist.",
            "metadata": {
                "propertyPath": "categorySetId"
            }
        }
    ]
}
