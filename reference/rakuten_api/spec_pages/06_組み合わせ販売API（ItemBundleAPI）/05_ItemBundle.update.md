...
URL: https://webservice.rms.rakuten.co.jp/merchant-portal/view/ja/common/1-1_service_index/itembundleapi/btoupdate/
サービス: 組み合わせ販売API（ItemBundleAPI）

サービス一覧へ戻る / ItemBundleAPI

RMS WEB SERVICE : ItemBundle.update
Overview

この機能を利用すると、組み合わせ販売を更新することができます。
...
2	表示設定	bundleState	yes	String	-	-	商品ページ上の組み合わせ表示設定。
...
・ACTIVE
・INACTIVE	ACTIVE
3	組み合わせ商品リスト	bundleItems	yes	List<bundleItem>	-	-	組み合わせられた商品のリスト。
親商品も該当リストに入ります。
1つの親商品に対して、子商品は必ず1つ登録する必要があり、最大3つまで登録できます。	
4	組み合わせ管理名称	bundleName	yes	String	32	-	組み合わせの管理名称	スマートフォンの組み合わせ
5	組み合わせ販売説明文	bundleDescription	no	String	50	-	組み合わせ販売の説明文	スマートフォン本体とスマホカバーの組み合わせです。
Level 2: bundleItem
No	Logical Name	Parameter Name	Required	Type	Max Char	Default	Description	Sample
...
・true
・false	true
3	商品削除フラグ	isDeletedItem	yes	Boolean	-	組み合わせ商品がデータベース上削除されたかどうかのフラグ。
...
1	エラー	errors	yes	List<Error>	-	組み合わせを更新する際に発生したエラーのリスト
Level 2: error
No	Logical Name	Parameter Name	Required	Type	Max Char	Description	Sample
...
Response in JSON format (Status: 200 OK)
{
    ...
    "updatedDate": "2017-11-30T06:30:00.000141Z"
}
値をnullに更新する場合（例：組み合わせ販売説明文をnullに更新）
Request (curl コマンドを使った例)
curl -X PUT \
  https://api.rms.rakuten.co.jp/es/1.0/bto/bundle/my-shop-bundle-01 \
  -H 'Authorization: ESA xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx' \
...
curl -X PUT \
  ...
  -d '{
    "bundleName": "スマートフォンの組み合わせ",
    "parentItemManageNumber": "item-001",
    "bundleDescription": "スマートフォン本体とスマホカバー・スクリーンガードの組み合わせです。",
    "bundleState": "ACTIVE",
    "bundleItems": [
        {
            "itemManageNumber": "item-001",
