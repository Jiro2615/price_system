...
URL: https://webservice.rms.rakuten.co.jp/merchant-portal/view/ja/common/1-1_service_index/inventoryapi2/searchbyinventory/
...
Overview
この機能を利用すると、在庫数の上限下限を指定し、商品管理番号、SKU管理番号、在庫数、登録日時、更新日時を最大1000件取得することができます。
更新日時の降順で出力されます。

Endpoint / HTTP Method
Endpoint	HTTP Method
https://api.rms.rakuten.co.jp/es/2.0/inventories/bulk-get/range	GET
Request
HTTP Header
No	Key	Value
...
失敗した場合
Request (curl コマンドを使った例)
curl --location --request GET 'https://api.rms.rakuten.co.jp/es/2.0/inventories/bulk-get/range?minQuantity=100000&maxQuantity=5000000' \
--header 'Authorization: ESA xxx'
